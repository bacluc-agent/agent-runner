...
Before generating the prompt, check `gh pr list -R bacluc-agent/agent-runner --state open --head issue-<number> --json number,updatedAt,headRefName`, falling back to `gh pr list -R bacluc-agent/agent-todo --state open --head issue-<number> --json number,updatedAt,headRefName` and then `gh pr list -R bacluc/provision-machines --state open --head issue-<number> --json number,updatedAt,headRefName` if the previous query returned empty (all queried with `--head issue-<n>`), and `gh issue view <number> -R bacluc-agent/agent-todo --json comments --jq '[.comments[] | select(.author.login != "bacluc-agent")] | max_by(.createdAt) | .createdAt // "none"'` to compare last human comment timestamp vs PR `updatedAt`. If an open `issue-<number>` branch/PR exists, forbid creating a duplicate branch/PR — improve the existing PR only when new human feedback exists (last-human-feedback newer than PR `updatedAt`). If PR is open and last-human-feedback is `none` or older than PR `updatedAt` (awaiting human feedback), skip it unless all other candidates are infeasible.

Known limitation: PRs living in other forks (e.g. `bacluc-agent/ecamp3`) or on suffixed head branches (e.g. `issue-198-review-pr-10773`, `issue-201-native-triggers`) are NOT detected by the exact `--head issue-<n>` query, so such issues may still be reported `[PR: none]` and re-picked; treat `[PR: none]` as "no PR found in the queried repos", not as proof no agent PR exists.
