...
3. **An issue is selected** (hourly, at minute 24). The _issue-selector_
   agent picks one ready issue, claims it with `agent-running:<run_id>` for the run, and
   writes the implementation prompt for the next step. The claim is removed
   when the run ends; stale claims from cancelled runs are also expired
   automatically at selection time.
...
| `hourly-issue.yml`             | hourly at minute 24                         | Picks one ready issue, claims it with `agent-running:<run_id>` for the run, and starts the implementation.                           |
